...
| AI 语音小助手 | [firmware/apps/ai_voice_assistant](firmware/apps/ai_voice_assistant/) | 按住 BOOT 录音，松手后上传到本机代理服务，并在墨水屏显示 AI 回复；默认支持 mock 代理，OpenAI Key 只放在 Mac 代理环境变量中 |
...
- `firmware/apps/ai_voice_assistant/` 是 AI 语音小助手应用；公开仓库默认不包含 Wi-Fi、代理 IP 或 OpenAI API Key。
