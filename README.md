...
当前稳定版本：`v0.1.0`，对应 AI 语音小助手第一版可用固件。
...
| AI 语音小助手 | [firmware/apps/ai_voice_assistant](firmware/apps/ai_voice_assistant/) | `v0.1.0` 稳定应用。按住 BOOT 录音，松手后保存、上传到本机 GLM 代理，并在墨水屏显示中文回复；内置 14px 中文字体、emoji 过滤和 Flash 存储回退 |
...
- `firmware/apps/ai_voice_assistant/` 是 AI 语音小助手应用；公开仓库默认不包含 Wi-Fi、代理 IP 或 GLM API Key。GLM Key 只应放在本机代理环境变量中。
